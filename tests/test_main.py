...
def test_speed_hundred_rows():
    ...
    s = time()
    fast_json_normalize(hundred_rows, to_pandas=True, order_to_pandas=True)
...
def test_speed_ten_thousand_rows():
    ...
    s = time()
    fast_json_normalize(ten_thousand_rows, to_pandas=True, order_to_pandas=True)
    fast_json_normalize_time_taken = time() - s
...
def test_speed_hundred_thousand_rows():
    ...
    s = time()
    fast_json_normalize(hundred_thousand_rows, to_pandas=True, order_to_pandas=False)
    fast_json_normalize_time_taken = time() - s
...
def test_speed_hundred_thousand_rows_ordered():
    ...
    print(f"\npandas time taken for a 100,000 rows: {pandas_json_normalize_time_taken}")
    s = time()
    fast_json_normalize(hundred_thousand_rows, to_pandas=True, order_to_pandas=True)
    fast_json_normalize_time_taken = time() - s
    print(
...
#     s = time()
#     pandas_json_normalize_time_taken = pd.json_normalize(million_rows)
#     print(f"\npandas time taken for a 100 rows: {time() - s}")
#     s = time()
#     fast_json_normalize_time_taken = fast_json_normalize(million_rows, to_pandas=True, order_to_pandas=True)
#     print(f"fast implementation time taken for a 100 rows: {time() - s}")
#     assert fast_json_normalize_time_taken < pandas_json_normalize_time_taken
